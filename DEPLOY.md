...
## 포트폴리오 공유 링크 · 면접 준비 · AI 사용량 상한

| 기능 | 설정 |
| --- | --- |
| 포트폴리오 공유 링크 | 없음. 포트폴리오 화면에서 링크를 만들면 `/p/<토큰>` 으로 로그인 없이 열립니다. 주소를 아는 사람만 볼 수 있고 검색에는 걸리지 않습니다(noindex). 언제든 주소를 새로 만들거나 공유를 중지할 수 있습니다. |
| 면접 준비 | 없음. 활동 상세의 면접 탭에서 예상 질문을 만들고 답변을 저장합니다. |
| AI 하루 상한 | `AI_DAILY_LIMIT` (선택) |

### AI 하루 상한 (실제 AI 키를 붙였다면 권장)

`ANTHROPIC_API_KEY` 를 넣는 순간 비용이 무제한으로 열립니다.
사용자 한 명이 하루에 쓸 수 있는 AI 호출 수를 정해두면 안전합니다.

| 이름 | Type | 값 |
| --- | --- | --- |
| `AI_DAILY_LIMIT` | Variable | 예: `30` (설정하지 않거나 0 이면 제한 없음) |

상한을 넘으면 AI 를 부르기 전에 막고 "오늘 사용 횟수를 모두 썼다"고 안내합니다.
